...
class QM_AdvancedFeedback_Helper_Callback extends Mage_Core_Helper_Abstract
{
    const SENDER_EMAIL_IDENTITY = 'qmadvfeedback/callback/sender_email_identity';

    public function canShowConfirmation()
    {
        return Mage::getStoreConfig('qmadvfeedback/callback/enable_confirmation');
    }

    public function getConfirmationText()
    {
        return Mage::getStoreConfig('qmadvfeedback/callback/confirmation_text');
    }
    ...
    public function getConfirmationHtml()
    {
        ...
        $confirmation = $layout->createBlock('core/template')
            ->setTemplate('qm/advfeedback/callback/confirmation.phtml')
    ...
    {
        return Mage::getStoreConfig('qmadvfeedback/callback/email_notification_enabled');
    }

    public function getEmailNotificationAddressList()
    {
        $addresses = Mage::getStoreConfig('qmadvfeedback/callback/send_email_notification_to');
        $addresses = explode("\n", $addresses);
        return $addresses;
    }
    ...
    public function sendNotifications($data)
    {
        if ($this->isEmailNotificationEnabled()) {
            ...
            $emailTemplate = Mage::getModel('core/email_template')->loadDefault('callback_email_template');
            $emailTemplate->setSenderEmail($this->_getSenderEmail());
            $emailTemplate->setSenderName($this->_getSenderName());
            $emailTemplate->setTemplateSubject('Subject: Callback');
