...
class QM_Checkout_Block_Block_Info extends Mage_Checkout_Block_Onepage_Abstract
{
    /**
    ...
    {
        $this->_order = $this->helper('qmcheckout')->buildSessionOrder();
    ...
    protected function _collectCustomerLocation()
    {
        ...
        if (!$this->helper('qmcheckout')->useOnlyDefaultCountry()) {
     ...
     *
     * @see QM_Checkout_Helper_Data::useOnlyDefaultCountry
     * @return string
     */
    ...
    public function getCountryBlock($type = 'shipping')
    {
        ...
        if (!$countryId) {
            $countryId = $this->helper('qmcheckout')->getDefaultCountryId();
        }
    ...
    public function getCustomerNote($subNote = null)
    {
        if (!$subNote) {
            return parent::getCustomerNote();
        }

        $customerNoteObject = new Varien_Object(parent::getCustomerNote());

        return $customerNoteObject->getData($subNote);
    }
}
