...
class QM_Checkout_Block_Payment_Methods extends Mage_Checkout_Block_Onepage_Payment_Methods
{
    /**
    ...
    {
        $result = $this->helper('qmcheckout')->isCentinelValidationRequired($method);
    ...
    public function getVerifyCcUrl(Mage_Payment_Model_Method_Abstract $method)
    {
        ...
        if ($this->isVerificationRequired($method)) {
            $verifyUrl = $this->getUrl('qm_order/checkout/verify');
        }
        return $verifyUrl;
    }
}
