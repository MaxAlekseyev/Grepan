...
class QM_Checkout_Model_Observer
{
    /**
     ...
     * @return QM_Checkout_Model_Observer
     */
    public function collectTotalsBefore(Varien_Event_Observer $observer)
    {
        ...
        Mage::helper('qmcheckout')->buildSessionOrder()
    ...
    public function addOrderComment(Varien_Event_Observer $observer)
    {
        ...
        if (!empty($data['note'])) {
            if (is_array($data['note'])) {
                $comment = '';
                foreach ($data['note'] as $name => $value) {
                    $name =  Mage::helper('qmcheckout')->__(ucwords(str_replace('_', ' ', $name)));
                    $comment .=  $name . ': ' . $value . '</br>';
                }
            } else {
