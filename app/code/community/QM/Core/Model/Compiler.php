...
class QM_Core_Model_Compiler
{
    public function extractor($node)
    {
        ...
        if (count($options) < count($modules)) {
            Mage::throwException(Mage::helper('qmcore')->__('There is not enough option(s) for specified module(s).'));
        }
        if (count($options) > count($modules)) {
            Mage::throwException(Mage::helper('qmcore')->__('There is not enough module(s) for specified option(s).'));
        }
    ...
    private function _rpn($conditions, $tokens)
    {
        ...
        for ($i = 0, $len = count($tokens); $i < $len; $i++) {
            if (is_numeric($tokens[$i])) {
                ...
                $outputStack[] = $condition !== null ? $condition : Mage::throwException(Mage::helper('qmcore')->__('Specified operands are more than condition(s).'));
            } else {
