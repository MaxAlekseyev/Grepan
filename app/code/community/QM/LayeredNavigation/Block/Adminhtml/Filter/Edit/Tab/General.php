...
class QM_LayeredNavigation_Block_Adminhtml_Filter_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Filter form
    ...
    protected function _prepareForm()
    {
        ...
        $model = Mage::registry('qm_layerednavigation_filter');
    ...
    protected function _prepareRegularForm()
    {
        ...
        $fldSet = $this->_form->addFieldset('qm_layerednavigation_general', array('legend' => $hlp->__('Display Properties')));
        ...
        if ($model && $model->getBackendType() == 'decimal') {
            ...
            $fldSet->addField('slider_decimal', 'text', array(
                'label' => $hlp->__('Number of digits after comma'),
                'name'  => 'slider_decimal',
            ));
            ...
            $fldSet->addField('display_type', 'select', array(
                'label'              => $hlp->__('Display Type'),
                'name'               => 'display_type',
                'values'             => Mage::helper('qm_layerednavigation')->getDisplayTypes(),
                'after_element_html' => $this->_getDisplayTypeScript('display_type')
            ));
            $fldSet->addField('show_search', 'select', array(
                'label'  => $hlp->__('Show Search Box'),
                'name'   => 'show_search',
                'values' => $yesno,
            ));
        ...
        $fldSet2 = $this->_form->addFieldset('qm_layerednavigation_blocks', array('legend' => $hlp->__('Additional Blocks')));
        ...
        $fldSet3 = $this->_form->addFieldset('qm_layerednavigation_seo', array('legend' => $hlp->__('Search Engines Optimization')));
        ...
        $fldSet4 = $this->_form->addFieldset('qm_layerednavigation_special', array('legend' => $hlp->__('Special Cases')));
    ...
    private function _getDisplayTypeScript($id)
    {
        $script = '';
        ...
        $script .= 'var selectEl = document.getElementById("'.$id.'");';
        $script .= 'var optionTabEl = document.getElementById("filterTabs_values");';

        $script .= 'function hideOptions() {';
        $script .= 'if (selectEl.value == 5 || selectEl.value == 6) {
                                optionTabEl.hide();
                            } else if (optionTabEl.style.display == "none") {
                                optionTabEl.style.display = "";
                            }';
        $script .= '}';

        $script .= 'if (selectEl && optionTabEl) {hideOptions();selectEl.onchange = hideOptions;}';
