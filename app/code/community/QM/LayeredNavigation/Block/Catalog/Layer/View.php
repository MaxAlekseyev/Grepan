<?php
class QM_LayeredNavigation_Block_Catalog_Layer_View extends Mage_Catalog_Block_Layer_View
{
    ...
    public function getFilters()
    {
    	...
    	if ($f->getPosition() > -1 && $this->_blockPos == Mage::getStoreConfig('qm_layerednavigation/block/stock_filter_pos')) {
        	$filters[] = $f;
        }
        // remove some filtes for the home page
        $exclude = Mage::getStoreConfig('qm_layerednavigation/general/exclude');
        ...
        $categoriesOrder = Mage::getStoreConfig('qm_layerednavigation/general/categories_order');
        ...
        foreach ($filters as $f){
        	if ($f instanceof QM_LayeredNavigation_Block_Catalog_Layer_Filter_Stock) {
        ...
        $optionsCollection = Mage::getResourceModel('qm_layerednavigation/value_collection')
        ...
        foreach ($filters as $f){
            ...
            elseif ($f instanceof Mage_Catalog_Block_Layer_Filter_Category){
                $f->setDisplayType(Mage::getStoreConfig('qm_layerednavigation/general/categories_type'));
                $f->setTemplate('qm/layerednavigation/category.phtml');
                $f->setHasSelection($catId != $this->getLayer()->getCurrentStore()->getRootCategoryId());
                $f->setCollapsed(Mage::getStoreConfig('qm_layerednavigation/general/categories_collapsed'));
            }
            elseif ($f instanceof Mage_Catalog_Block_Layer_Filter_Price){
                $attrCode = $f->getAttributeModel()->getAttributeCode();
                if ('price' == $attrCode){
                    $f->setDisplayType(Mage::getStoreConfig('qm_layerednavigation/general/price_type'));
                    $f->setTemplate('qm/layerednavigation/price.phtml');
                    $f->setSliderType(Mage::getStoreConfig('qm_layerednavigation/general/slider_type'));
                    ...
                    $f->setFromToWidget(Mage::getStoreConfig('qm_layerednavigation/general/price_from_to'));
                    $f->setAttributeCode($attrCode);
                    $f->setCollapsed(Mage::getStoreConfig('qm_layerednavigation/general/price_collapsed'));
                    $f->setSeoRel(Mage::helper('qm_layerednavigation')->getSeoPriceRelNofollow());
                }
            }
    ...
    protected function _getFilterableAttributes()
    {
        ...
        if (is_null($attributes)) {
            ...
            $attributes = clone Mage::helper('qm_layerednavigation/attributes')->getFilterableAttributes();
            foreach ($attributes as $k => $v){
                ...
                elseif($v->getAttributeCode() == 'price'){
                    $pos = Mage::getStoreConfig('qm_layerednavigation/block/price_pos');
                }
    ...
    {
        $pos = Mage::getStoreConfig('qm_layerednavigation/block/state_pos');
        if ($this->_notInBlock($pos)){
            return '';
        }
        $this->getChild('layer_state')->setTemplate('qm/layerednavigation/state.phtml');
    ...
    protected function _afterToHtml($html)
    {
        ...
        foreach ($this->getFilters() as $f){
            ...
            if ($f->getCollapsed() && !$f->getHasSelection()){
                $html = str_replace('<dt>'.$name, '<dt class="layerednavigation-collapsed">'.$name, $html);
            }
            $comment = $f->getComment();
            if ($comment){
                $img = Mage::getDesign()->getSkinUrl('images/qm_layerednavigation/layerednavigation-tooltip.png');
                $img = '<img class="layerednavigation-tooltip-img" src="'.$img.'" width="9" height="9" alt="'.htmlspecialchars($comment).'" id="layerednavigation-img-'.$f->getAttributeCode().'"/>';
    ...
    {
        $pos = Mage::getStoreConfig('qm_layerednavigation/block/categories_pos');
        if ($this->_notInBlock($pos)){
            $this->_categoryBlockName = 'qm_layerednavigation/catalog_layer_filter_empty';
        }        
        if (Mage::getStoreConfig('qm_layerednavigation/block/stock_filter') >= 0) {
        	$stockBlock = $this->getLayout()->createBlock('qm_layerednavigation/catalog_layer_filter_stock')
        ...
        if (Mage::registry('qm_layerednavigation_layout_prepared')){
            return parent::_prepareLayout();
        }
        else {
            Mage::register('qm_layerednavigation_layout_prepared', true);
        }
        
        if (!Mage::getStoreConfigFlag('customer/startup/redirect_dashboard')) { 
            $url = Mage::helper('qm_layerednavigation/url')->getFullUrl($_GET);
        ...
        if ($head){
            $head->addJs('qm_layerednavigation/layerednavigation.js');
             
            if (Mage::getStoreConfigFlag('qm_layerednavigation/block/ajax') && !$this->getIsProductPage()){
                $head->addJs('qm_layerednavigation/layerednavigation-ajax.js');
            }
        }
    ...
    {
        $attributes = Mage::registry('qm_layerednavigation_attributes');
        if (!$attributes){
            //additional filter properties
            $attrCollection = Mage::getResourceModel('qm_layerednavigation/filter_collection')
                ->load();
    ...
    {
        if (Mage::helper('qm_layerednavigation')->isVersionLessThan(1, 4, 2)){
            $pos = Mage::getStoreConfig('qm_layerednavigation/block/categories_pos');
            if ($this->_notInBlock($pos)){
                $this->_categoryBlockName = 'qm_layerednavigation/catalog_layer_filter_empty';
