<?php
class QM_LayeredNavigation_Block_List extends Mage_Core_Block_Template
{
    private $items = array();
    ...
    protected function _prepareLayout()
    {
        ...
        $modelName  = Mage::helper('qm_layerednavigation')->isVersionLessThan(1, 4) ? 'catalog/entity_attribute' : 'catalog/resource_eav_attribute';
        ...
        $collection = Mage::getResourceModel('qm_layerednavigation/value_collection')
            ->addFieldToFilter('option_id', array('in'=>$ids))
            ->load();
        $images = array();
        foreach ($collection as $value){
            $images[$value->getOptionId()] = $value->getImgBig() ? Mage::getBaseUrl('media') . 'qm_layerednavigation/' . $value->getImgBig() : '';
        }
        // end add images        
