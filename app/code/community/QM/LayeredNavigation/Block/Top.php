<?php
class QM_LayeredNavigation_Block_Top extends Mage_Core_Block_Template
{
    ...
    {
        return 'layerednavigation-filters-wrapper';
    }        


    protected function _isPageHandled()
    {
        $page = Mage::getModel('qm_layerednavigation/page');
        ...
        $titleSeparator = Mage::getStoreConfig('qm_layerednavigation/general/title_separator');
        $descrSeparator = Mage::getStoreConfig('qm_layerednavigation/general/descr_separator');
        $kwSeparator = ',';
    ...
    protected function _prepareLayout()
    {
        ...
        $hasCanonical = !Mage::helper('qm_layerednavigation')->isVersionLessThan(1, 4);
        if ($hasCanonical){
            $url = Mage::getSingleton('catalog/layer')->getCurrentCategory()->getUrl();
            ...
            $isShopby = in_array(Mage::app()->getRequest()->getModuleName(), array(Mage::getStoreConfig('qm_layerednavigation/seo/key'), 'qm_layerednavigation'));
            if ($isShopby){
                $url = '';
            }
            $head->addLinkRel('canonical', Mage::helper('qm_layerednavigation/url')->getCanonicalUrl($url));
        }
        ...
        $filters = Mage::getResourceModel('qm_layerednavigation/filter_collection')
                ->addTitles()
                ->setOrder('position');
        $hash = array();
        ...
        foreach ($filters as $f){
            ...
            $vals = Mage::helper('qm_layerednavigation')->getRequestValues($code);
            if ($vals){
                foreach($vals as $v){
        ...
        if ($priceVals) {
            if (Mage::helper('qm_layerednavigation')->getSeoPriceNofollow()){
                $robotsFollow = 'nofollow';
            }
            if (Mage::helper('qm_layerednavigation')->getSeoPriceNoindex()){
                $robotsIndex = 'noindex';
            }
        }
         ...
         */
        $catNoIndex = Mage::getStoreConfig('qm_layerednavigation/seo/cat_noindex_nofollow');
        if ($catNoIndex != '') {
        ...
        $options = Mage::getResourceModel('qm_layerednavigation/value_collection')
        ...
        if ($head){
            ...
            foreach ($hash as $opt){
            	if (isset($query[Mage::getStoreConfig('qm_layerednavigation/brands/attr')])) {
        			if ($opt->getOptionId() == $query[Mage::getStoreConfig('qm_layerednavigation/brands/attr')]) {
        				$breadcrumbs = $this->getLayout()->getBlock('breadcrumbs');
						$breadcrumbs->addCrumb('layerednavigation-brand', array('label' => $opt->getTitle(), 'title' => $opt->getTitle()));
        			} 
		        }
    ...
    public function getOptions()
    {
        ...
        foreach ($this->options as $opt){
            ...
            $item['image'] = '';
            if ($opt->getImgBig()){
                $item['image'] = Mage::getBaseUrl('media') . '/layerednavigation/' . $opt->getImgBig();
            }
            $res[] = $item;
        }
        return $res;
...
/**
    ...
    public function handleExtraAttributes()
    {
        ...
        if ($head){
        	...
        	if (Mage::getStoreConfig('qm_layerednavigation/seo/prev_next')) {
        		...
        		if ($tool) {
				     ...
				     */
				    $currenUrl = Mage::helper('qm_layerednavigation/url')->getFullUrl();
        	...
        	if (Mage::app()->getRequest()->getParam('price')) {
	        	$robotsIndex = Mage::getStoreConfig('qm_layerednavigation/general/price_tag_noindex');
	        	$robotsFollow = Mage::getStoreConfig('qm_layerednavigation/general/price_tag_nofollow');
