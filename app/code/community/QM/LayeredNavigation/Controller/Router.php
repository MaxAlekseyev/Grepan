<?php
class QM_LayeredNavigation_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    const MIDDLE    = 0;
    const BEGINNING = 1;
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        $suffix = Mage::helper('qm_layerednavigation/url')->getUrlSuffix();
        if ($suffix && '/' != $suffix){
        ...
        if ($params){
            Mage::register('qm_layerednavigation_current_params', $params);
        }
        ...
        if ($cat){ // normal category
            ...
            $realModule = 'QM_LayeredNavigation';

            $request->setPathInfo(trim($reservedKey, '/'));
            $request->setModuleName('qm_layerednavigation');
            $request->setRouteName('layerednavigation');
    ...
    {
        $code = Mage::getStoreConfig('qm_layerednavigation/brands/attr');

        if (!$code) {
            return false;
        }

        $options = Mage::helper('qm_layerednavigation/url')->getAllFilterableOptionsAsHash();
        ...
        foreach ($options[$code] as $key => $id) {
            if (!Mage::getStoreConfig('qm_layerednavigation/seo/hide_attributes')){
                $key = $code . Mage::getStoreConfig('qm_layerednavigation/seo/option_char') . $key;
