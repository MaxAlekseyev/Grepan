<?php
class QM_LayeredNavigation_Helper_Data extends Mage_Core_Helper_Abstract
{
    protected $filters = null;
    protected $icons   = null;

    const XML_PATH_SEO_PRICE_NOFOLLOW       = 'qm_layerednavigation/seo/price_nofollow';
    const XML_PATH_SEO_PRICE_NOINDEX        = 'qm_layerednavigation/seo/price_noindex';
    const XML_PATH_SEO_PRICE_RELNOFOLLOW    = 'qm_layerednavigation/seo/price_rel_nofollow';
    ...
    public function getDisplayTypes()
    {
        ...
        if ($this->isColorSwitherEnabled()) {
            array_push($displayTypes, $this->__('Images Only from QM Switcher'));
            array_push($displayTypes, $this->__('Images and Labels from QM Switcher'));
        }

        return $displayTypes;
    }

    public function isColorSwitherEnabled()
    {
        return Mage::helper('core')->isModuleEnabled('QM_Switcher');
    }
    ...
    protected function _getFilters()
    {
        if (is_null($this->filters)){
            ...
            $filterCollection = Mage::getResourceModel('qm_layerednavigation/filter_collection')
    ...
    public function init()
    {
        ...
        $optionCollection = Mage::getResourceModel('qm_layerednavigation/value_collection')
            ->addPositions()
        ...
        foreach ($optionCollection as $opt){
            ...
            $this->icons[$opt->getOptionId()] = array(
                ...
                'title' => $opt->getTitle(),
                'descr' => $opt->getDescr(),
                'img'   => Mage::getBaseUrl('media') . 'layerednavigation/' . $img,
    ...
    public function showLinks($product, $mode='view', $names=array(), $exclude=false)
    {
        ...
        usort($items, array('QM_LayeredNavigation_Helper_Data', '_srt'));

        //create block
        $block = Mage::getModel('core/layout')->createBlock('core/template')
            ->setArea('frontend')
            ->setTemplate('qm/layerednavigation/links.phtml');
