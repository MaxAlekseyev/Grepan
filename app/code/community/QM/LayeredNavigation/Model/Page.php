<?php
class QM_LayeredNavigation_Model_Page extends Mage_Core_Model_Abstract
{
    ...
    public function match()
    {
        ...
        foreach ($cond as $k => $v) {
            ...
            $vals = Mage::helper('qm_layerednavigation')->getRequestValues($k);
            if (is_array($v)) {
    ...
    {
        $attributes = Mage::getModel('qm_layerednavigation/filter')->getResourceCollection()->addFrontendInput($attributeCode);
        return $attributes->getFirstItem();
    }
        
    public function getOptionsForFilter($attributeCode, $frontendInput)
    {
        $filters = Mage::getModel('qm_layerednavigation/filter')->getResourceCollection()->addFrontendInput($attributeCode);
        $filterId = $filters->getFirstItem()->getFilterId();
        
        $options = Mage::getModel('qm_layerednavigation/value')->getResourceCollection()->addFilter('filter_id', $filterId);
