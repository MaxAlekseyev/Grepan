...
class QM_Switcher_Block_Adminhtml_System_Attribute_Renderer extends Mage_Core_Block_Template
{
    /**
     * path to transform config
     */
    const XML_TRANSFORM_OPTIONS_PATH = 'qm_switcher/system/transform';
    /**
     ...
     * @var string
     */
    protected $_template = 'qm/switcher/system/config/form/field/attribute.phtml';
    ...
    public function getForm()
    {
        if (is_null($this->_form)) {
            ...
            $fieldset = $this->_form->addFieldset(
                ...
                array(
                    'legend' => Mage::helper('qm_switcher')->__('Configurable attributes transforms')
                )
            );
            $fieldset->addField(
                ...
                'label',
                array(
                    'label' => Mage::helper('qm_switcher')->__('Attribute'),
                    'value' => Mage::helper('qm_switcher')->__('Transformation'),
                )
            );
    ...
    public function getAfterHtml(Mage_Catalog_Model_Resource_Eav_Attribute $attribute)
    {
        ...
        foreach ($this->getTransformationOptions() as $key => $option) {
            if ((string)$option->after_block) {
                /** @var QM_Switcher_Block_Adminhtml_System_Attribute_After_Abstract $block */
                $block = Mage::app()->getLayout()->createBlock((string)$option->after_block);
                if (!($block instanceof QM_Switcher_Block_Adminhtml_System_Attribute_After_Abstract)) {
                    throw new QM_Switcher_Exception(
                        "After block is not an instance of QM_Switcher_Block_Adminhtml_System_Attribute_After_Abstract"
                    );
                }
