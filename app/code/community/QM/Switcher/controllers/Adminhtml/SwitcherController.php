...
class QM_Switcher_Adminhtml_SwitcherController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    public function configureAction()
    {
        ...
        switch($type) {
            ...
            default:
                ...
                Mage::dispatchEvent('qm_switcher_configure_transform', array('result' => $resultObject));
    ...
    protected function _configureColor($attributeId, $values)
    {
        return $this->_configureElements(
            $attributeId,
            'color',
            Mage::helper('qm_switcher')->__('Colors'),
            'text',
            $values
        );
    }
    ...
    protected function _configureImage($attributeId, $values)
    {
        return $this->_configureElements(
            $attributeId,
            'image',
            Mage::helper('qm_switcher')->__('Images'),
            'image',
            $values
        );
    }
    ...
    protected function _configureElements($attributeId, $suffix, $label, $fieldType, $values)
    {
        ...
        if (!$attribute || !$attribute->getId()) {
            $result = array(
                'success' => false,
                'html' => Mage::helper('qm_switcher')->__(
        ...
        $fieldset->addType('image', Mage::getConfig()->getBlockClassName('qm_switcher/adminhtml_helper_image'));
    ...
    public function saveImageAction()
    {
        ...
        $errors = array();
        $destinationFolder = Mage::helper('qm_switcher/optimage')->getImageBaseDir();
        foreach ($_FILES as $key => $data) {
            if (!empty($_FILES[$key]['tmp_name'])) {
                try {
                    ...
                    $errors[Mage::helper('qm_switcher')->__('There was a problem uploading images')] = 1;
                }
            } else {
