    ...
    foreach ($toTransform as $id => $code) {
        ...
        if (isset($usesColors[$id])) {
            ...
            $newConfigArray['options'][$id]['product_image'] = $this->getConfigSettings(
                ...
                $scope,
                $this->getDefaultValue('qm_switcher/settings/image_attribute')
            );
        } else {
            $newConfigArray[$id] = 'labels';
        }
    }
    $configPath = QM_Switcher_Block_Catalog_Product_View_Type_Configurable_Config::XML_TRANSFORM_PATH;
