	...
	public function getPriceProperties($product) {
		...
		if ($productTypeId === 'bundle') {
			if ($product->getPriceType() == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
				$minimalPrice = Mage::helper('tax')->getPrice($product, round(Mage::helper('core')->currency($product->getFinalPrice(),false)), $includeTax);
			...
			$html .= '<meta itemprop="price" content="' . Mage::helper('tax')->getPrice($product, round(Mage::helper('core')->currency($product->getFinalPrice(),false)), $includeTax) . '" />';
		}
