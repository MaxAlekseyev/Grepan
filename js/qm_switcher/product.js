...
QM.Switcher = Class.create(Product.Config, {
    currentValues: {},
    rewritten: false,
    /**
    ...
    transformDropdown: function (selectid)
    {
        ...
        if (switchConfig && typeof(switchConfig) == "object") {
			...
			$(selectid).fire('qm_switcher:transform');
        }
        return transformed;
    },
    /**
    ...
    getOptionText: function (elem, value, config)
    {
        ...
        if (!configType || configType == 'labels') {
            return text;
        }
    ...
    getConfigValue: function (config, path, def)
    {
        ...
        var i = 0;
        while (i < parts.length && cloneConfig != -1 && cloneConfig != null) {
            var part = parts[i];
